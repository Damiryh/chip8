...
std::map<sf::Keyboard::Key, ch8::byte> key_map {
	{sf::Keyboard::Num3, 0x3},
	{sf::Keyboard::Up, 0x2},
	{sf::Keyboard::Left, 0x4},
	{sf::Keyboard::Right, 0x6},
	{sf::Keyboard::Down, 0x8},
};
*/
...
void reset() {
	...
	pps = 15000.0; 	// инструкций в секунду
	...
	while (window.isOpen()) {
		...
		while (window.pollEvent(event)) {
			...
			if (event.type == sf::Event::KeyPressed) {
				if (event.key.code == sf::Keyboard::Key::Backspace) { reset(); }
				else { keyboard.press(event.key.code); }
			} else
		...
		if (ft > 1/tps) {
			...
			// Заодно обновляем состояние бипера
			if (processor.beep()) {
		...
		if (tt > 1/fps) {
			...
			screen.draw(); window.display();
		}
		
		for (int i = 0; i < dt*pps; i++) {
			processor.tick();
		}
	}
